...
t1 :     printf("Enter string length for each test case\n");
	   scanf("%d",&t);
	if(t <= 0 || t>= (10^3))			// check condition for test cases
	{
	 printf("you entered invalid test case no.\tre-enter no. of test cases \n");
	   /*char ans;
	scanf("%c",&ans);
            if(ans == 'y' || ans == 'Y')*/
		goto t1;
...
void MakeBitString(int n,char string[],int k,int *counter,int tcn)	//create bit string to length l with given conditions
{
	...
	else if(string[k-1]=='0')
	{          //no two should be adjacent so that if previous position contains 0 then current position holds 1
		string[k] = '1';
